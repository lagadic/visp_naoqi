...
vpMatrix vpNaoqiRobot::get_eJe(const std::string &chainName, vpMatrix &tJe) const
{
    ...
    else if (chainName == "LEye_t") // Consider the trunk
    {
        ...
        //Get the index of the position of the q of the first joint of the chain in the confVector
        int index_confVec_trunk = (boost::fusion::at_c<RomeoModel::TrunkYaw_link>(robot.nodes).q_idx);
        ...
        std::cout << "index_confVec_trunk: " <<std::endl << index_confVec_trunk;
        ...
        // Now we want to transform tJe to eJe
        vpHomogeneousMatrix tMREye(m_motionProxy->getTransform(jointNamesEye[jointNamesEye.size()-1], 0, true));// get transformation  matrix base torso and LEye
        vpHomogeneousMatrix tMTrunk(m_motionProxy->getTransform("TrunkYaw", 0, true));// get transformation  matrix base torso and TrunkYaw


        vpVelocityTwistMatrix REyeVTrunk(tMREye.inverse()*tMTrunk);
    ...
    else if (chainName == "REye_t") // Consider the trunk
    {
#ifdef VISP_NAOQI_HAVE_MATAPOD
        //Jacobian matrix w.r.t the torso
        //vpMatrix tJe;
        ...
        // Get the names of the joints in the chain we want to control (Trunk + Head + Eye)
        std::vector<std::string> jointNamesTrunkHead;
        jointNamesTrunkHead.push_back("TrunkYaw");
        std::vector<std::string> jointNamesHead = m_motionProxy->getBodyNames("Head");
        jointNamesTrunkHead.insert(jointNamesTrunkHead.end(),jointNamesHead.begin(),jointNamesHead.end());
        std::vector<std::string> jointNamesEye = m_motionProxy->getBodyNames("REye");

        //jointNames.insert(jointNames.end(), jointNamesEye.begin(), jointNamesEye.end());

        // Get the angles of the joints in the chain we want to control

        std::vector<float> qmp_trunk_head = m_motionProxy->getAngles("TrunkYaw",true);
        std::vector<float> qmp_head = m_motionProxy->getAngles(jointNamesHead,true);
        qmp_trunk_head.insert(qmp_trunk_head.end(),qmp_head.begin(),qmp_head.end());
        std::vector<float> qmp_leye = m_motionProxy->getAngles(jointNamesEye,true);


        const unsigned int nJoints= qmp_trunk_head.size() + qmp_leye.size();
        ...
        // Copy the angle values of the joint in the confVector in the right position
        // In the first 6 positions there is the FreeFlyer. We used the index_confVec to copy the rigth values.
        for(unsigned int i=0;i<jointNamesTrunkHead.size();i++)
            q[i+index_confVec] = qmp_trunk_head[i];
        ...
        // Copy jacobian Head
        for(unsigned int i=0;i<3;i++)
            for(unsigned int j=0;j<jointNamesTrunkHead.size();j++)
            {
                tJe[i][j]=J(i+3, index_confVec +j);
                tJe[i+3][j]=J(i, index_confVec +j);
            }
        ...
        // Now we want to transform tJe to eJe
        vpHomogeneousMatrix baseMREye(m_motionProxy->getTransform(jointNamesEye[jointNamesEye.size()-1], 2, true));// get transformation  matrix base torso and LEye
        vpVelocityTwistMatrix REyeVLbase(baseMREye.inverse());
